...
def encode_categorical(df, one_hot_min=2,
                       onehot_max=10,
    ...
    for c in label_e_columns:
        ...
        if label_encoders_train is None or c not in label_encoders_train.keys():
            ...
            le = OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1, dtype=np.int)
    ...
    df = pd.get_dummies(df, columns=one_hot_columns, dummy_na=False)  # dummy_na=True - to check for not filled

    # -- numerical with two columns
    print("Two values with NA")
    for c in numerical_columns:
...
def encode_categorical_pipe(p, id_check: str,
                            one_hot_min=2,
                            onehot_max=10,
                            label_encoders_train: dict = None, id_check_f=True) -> (str, list):
    df: pd.DataFrame = pd.read_pickle(p)

    df, label_encoders = encode_categorical(df, one_hot_min=one_hot_min, onehot_max=onehot_max, label_encoders_train=label_encoders_train)
